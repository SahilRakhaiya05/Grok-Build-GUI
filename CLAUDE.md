...
| `scripts/uninstall.{ps1,sh}` | Uninstall `grok-gui.grok-vscode-gui` |
...
npm run package  # → grok-vscode-gui-X.Y.Z.vsix (clears older *.vsix first)
```
...
5. **GitHub Release** for that tag → `gh release create vX.Y.Z --title "Release vX.Y.Z" --notes-file <notes> <vsix>` (notes = the new changelog section(s); include any earlier version that was bumped but never released). **Always attach the built `grok-vscode-gui-X.Y.Z.vsix` as a release asset** so the exact installable build is downloadable from the release.
6. **Marketplace publish is separate and explicit** — only `npm run publish` (vsce) when the user asks. The `grok-gui` publisher is used for publishing; publishing ≠ tagging.
...
- **Rebuilding clears older `.vsix` first** — `npm run package` (and the install/release scripts) delete stale `grok-vscode-gui-*.vsix` before building, so only the current version is on disk. After any doc or code change, rebuild + reinstall so the installed extension's bundled docs are current.
